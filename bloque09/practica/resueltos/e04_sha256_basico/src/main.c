...
/* Constantes SHA-256 definidas por estándar FIPS 180-4. */
...
/* Compresión SHA-256 sobre bloque de 512 bits (64 bytes). */
...
static void sha256_final(sha256_ctx_t *ctx, uint8_t out[32]) {
    ...
    /* Padding: byte 0x80 y luego ceros hasta byte 56 del bloque. */
    ctx->data[i++] = 0x80U;
    if (i > 56U) {
        while (i < 64U) {
            ctx->data[i++] = 0x00U;
    ...
    /* Longitud total del mensaje en bits, big-endian. */
...
int main(int argc, char **argv) {
    const char *msg = (argc >= 2) ? argv[1] : "abc";
    ...
    printf("msg=%s sha256=%s\n", msg, hex);

    /* Vector estándar: SHA256("abc") */
    const char *expected = "ba7816bf8f01cfea414140de5dae2223"
                           "b00361a396177a9cb410ff61f20015ad";

    return (strcmp(msg, "abc") == 0 && strcmp(hex, expected) == 0) ? EXIT_SUCCESS : EXIT_FAILURE;
}
