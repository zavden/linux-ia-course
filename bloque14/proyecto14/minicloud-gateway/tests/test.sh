...
../minicloud-registry/build/main --serve "$REG_PORT" tests/data/routes.sample >/tmp/b14-gw-reg.log 2>&1 &
PID_REG=$!
../minicloud-vault/build/main --serve "$VAULT_PORT" tests/data/secrets.sample >/tmp/b14-gw-vault.log 2>&1 &
PID_VAULT=$!
../minicloud-runner/build/main --serve "$RUN_PORT" tests/data/capacity.sample >/tmp/b14-gw-run.log 2>&1 &
PID_RUN=$!
../minicloud-monitor/build/main --serve "$MON_PORT" >/tmp/b14-gw-mon.log 2>&1 &
PID_MON=$!
...
REP=$(../minicloud-monitor/build/main --client 127.0.0.1 "$MON_PORT" GET /report)
